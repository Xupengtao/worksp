...
enum CLLinkStatus {Opening,HeadLinked,RearLinked,Linked};
...
CircularList<Type>::CircularList(int Length):Length_(Length+1)
{
	...
	ReadInd = 0;
	CircularListNum++;
#if DEBUG_CIRCULARLIST
	NLogF("Creating... No.%d CircularList",CircularListNum);
#endif
}
...
{
	if((LinkStatus != RearLinked)&&(LinkStatus != Linked)&&(NextChain.LinkStatus != HeadLinked)&&(NextChain.LinkStatus != Linked))
	{
		pNextChain = NextChain.GetArrayHeadPoint();
		Array[Length_-1].next = pNextChain;
		if(LinkStatus == HeadLinked)
		{
			LinkStatus = Linked;
		}
		else
		{
			LinkStatus = RearLinked;
		}
		if(NextChain.LinkStatus == RearLinked)
		{
			NextChain.LinkStatus = Linked;
		}
		else
		{
			NextChain.LinkStatus = HeadLinked;
		}
		return 0;
	}
	else
...
#if DEBUG_CIRCULARLIST
	NLogF("WriteReset Ind = %d",pWrite->Index);
#endif
}
...
void CircularList<Type>::ReadReset()
{
	...
	ReadInd = 0;
#if DEBUG_CIRCULARLIST
	NLogF("ReadReset Ind = %d",pRead->Index);
#endif
}
...
{
	NLogF("Array[%d].data = %d",Ind,Array[Ind].data);
}
...
void CircularList<Type>::show(const int BeginInd ,const int EndInd)
{
	if(EndInd <= BeginInd)
	{
		NLogF("CircularList show Invalid!");
	}
	else
	{
		for(unsigned int i=0;i<EndInd-BeginInd+1;i++)
		{
			NLogF("Array[%d].data = %d",BeginInd+i,Array[BeginInd+i].data);
		}
	}
}
