...
public:
    smart_ptr_t() : m_managed(0), m_use_count(0){}
    explicit smart_ptr_t(T* rep) : m_managed(rep), m_use_count(new unsigned int(1)){}
    smart_ptr_t(const smart_ptr_t& r) : m_managed(0), m_use_count(0)
    {
        m_managed = r.get();
    ...
    template<class Y>
    smart_ptr_t(const smart_ptr_t<Y>& r) : m_managed(0), m_use_count(0)
    {
        m_managed = r.get();
        m_use_count = r.use_count_ptr();
        if(m_use_count)
