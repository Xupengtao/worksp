...
public:
    _ElectromagneticVision(UINT rows = 1024, UINT cols = 1024)
                         : _PdwToMatType(rows, cols),
                           ...
                           VideoFrameMat(cols + 400, 3 * rows + 600, CV_8UC3, cv::Scalar::all(255)),
    ...
    void OpenFile(const string& filepath, CINT AddTime_ms, CINT ToaUnit_ns = 80)
    {
        _PdwToMatType::OpenFile(filepath);
        PixelToaUnit    = float(AddTime_ms)/this->AddCol;
    ...
    Mat& ProcessFrame()
    {
        ...
        // Analysis
        this->PdwImageAnalysis(BGRMat, DensityDilateMat, this->PdwRfPwMapClt, cvTextMat, PixelToaUnit);     // 图像分析
        // Generage VideoFrame
