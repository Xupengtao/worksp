/*
 * @Author: xpt 
 * @Date: 2019-12-23 17:26:56 
 * @Last Modified by: xpt
 * @Last Modified time: 2019-12-26 18:41:25
 */
...
private:
    ...
    _BMPFileHeader m_file_header;    //BMP图像的文件头
    _BMPInfoHeader m_info_header;    //BMP图像的信息头
    uint m_color_palette_size;       //BMP的调色板数组大小
    ColorPalette *m_color_palette;   //调色板(Color Palette)
...
public:
     ...
     */
    explicit BMPImage():m_width(0),m_height(0),m_depth(8),m_row_size(0),
     ...
     */
    explicit BMPImage(const char* bmp_file_path):m_data(nullptr), m_color_palette(nullptr)
    {
        char buffer[500];
     ...
     */
    explicit BMPImage(int4 width, int4 height, uint2 depth = 24):m_data(nullptr), m_color_palette(nullptr)
    {
    ...
    bool resize(int4 width, int4 height)
    {
        uint2 depthTmp = m_depth;
        init();
        m_width  = width;
        m_height = height;
        m_depth  = depthTmp;
    ...
    bool open(const char* bmp_file_path)
    {
        ...
        if (m_file_header.bfType != 0x4d42) // 0x4d42 = 'BM'      // 判断是否是bmp图像
        {
        ...
        {
            m_color_palette = new ColorPalette[m_color_palette_size];
            im_file.read((char*)(m_color_palette), m_color_palette_size * sizeof(ColorPalette));    // 读取调色板数据
        }
        m_width = m_info_header.biWidth;                        // 给属性赋值
        m_height = m_info_header.biHeight;
        m_depth = m_info_header.biBitCount;
        m_row_size = 4u * ((m_info_header.biBitCount * m_info_header.biWidth+31u)/32u);             // 计算每行的字节数
        // ---------------------------读取图像数据--------------------------------
        m_data = new uchar[m_info_header.biSizeImage]();                                            // 申请内存空间并同时初始化为0
        im_file.read((char *)m_data,m_info_header.biSizeImage);                                     // 读取图像数据到内存
        im_file.close();                                        // 关闭文件
        return true;
    }
    ...
    bool clear(UCHAR PixelInit = 0)
    {
        memset(m_data, PixelInit, m_info_header.biSizeImage);
    }
    int4 width() 
    {
        return abs(m_width);
    ...
    void ImageStatus(int ArixSize = 5)
    {
        COUTSWIDTH(ArixSize, "文件类型", m_file_header.bfType,
                             "文件大小", m_file_header.bfSize,
                             "从文件头到位图数据的偏移", m_file_header.bfOffBits);
        COUTSWIDTH(ArixSize, "信息头的大小", m_info_header.biSize,
                             "宽度", m_info_header.biWidth,
                             "高度", m_info_header.biHeight,
                             "颜色平面数", m_info_header.biPlanes,
                             "比特数/像素数", m_info_header.biBitCount,  
                             "压缩类型", m_info_header.biCompression);
        COUTSWIDTH(ArixSize, "位图数据的大小", m_info_header.biSizeImage, 
                             "水平分辨率", m_info_header.biXPelsPerMeter,
                             "垂直分辨率", m_info_header.biYPelsPerMeter,
                             "调色板中的颜色索引数", m_info_header.biClrUsed,
                             "有重要影响的颜色索引数", m_info_header.biClrImportant);
        COUTSWIDTH(ArixSize, "BMP的调色板数组大小", m_color_palette_size);
    }
    void showBGRA(ColorPalette &BGRATmp)
    {
        COUT(BGRATmp.b, BGRATmp.g, BGRATmp.r, BGRATmp.a);
    }
    void showColorPalette(int st = 0, int ed = 10)
    {
        if(st >= m_color_palette_size)
        {
            COUTSTREND("st >= m_color_palette_size");
            st = 0;
        }
        if(ed > m_color_palette_size)
        {
            COUTSTREND("ed > m_color_palette_size");
            ed = m_color_palette_size;
        }
        if(m_color_palette == nullptr)
        {
            COUTSTREND("m_color_palette == nullptr");
            return;
        }
        for(int i = st; i < ed; i++)
        {
            showBGRA(m_color_palette[i]);
        }
    }
    void showMPGData(int st = 0, int ed = 10)
    {
        if(st >= m_info_header.biSizeImage)
        {
            COUTSTREND("st >= m_info_header.biSizeImage");
            st = 0;
        }
        if(ed > m_info_header.biSizeImage)
        {
            COUTSTREND("ed > m_info_header.biSizeImage");
            ed = m_info_header.biSizeImage;
        }
        if(m_data == nullptr)
        {
            COUTSTREND("m_data == nullptr");
            return;
        }
        for(int i = st; i < ed; i++)
        {
