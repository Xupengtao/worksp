...
#                return -2
        if startsign != 1:
            return -1
        stage = self.dgraph_stage(dgraph,startlable)
        return stage
#        layernodeindex = 0                                                              #当前优化层节点数  居中处理
#        dgraphpos[nodetemp] = np.array([self.layerW * layernodeindex, self.layerH * (100 - (layertemp + 1))])
    ...
    def dgraph_stage(self,dgraph,startlable):                                       #节点分层 
        nodes = dgraph.nodes()                                                          #节点表
        edges = dgraph.edges()                                                          #边列表
        stage = [0] * 1000                                                              #层包含节点列表
        nodestage = {}                                                                  #节点的父节点及对应层字典
        nodetemp = startlable                                                           #当前节点
        layertemp = 0                                                                   #当前层
        editednode = []                                                                 #已删除节点表
        stage[layertemp] = [0,[]]
        stage[layertemp][0] = stage[layertemp][0] + 1
        stage[layertemp][1].append(nodetemp)
        nodestage[nodetemp] = ['',layertemp]
        nodes.remove(nodetemp)
        editednode.append(nodetemp)
        layertemp = layertemp + 1
        stage[layertemp] = [0,[]]
        while nodes != []:
            editnodetemp = len(editednode)
            for nodetemp in stage[layertemp-1][1]:
                for edge in edges:                                                      #查找当前节点对应子节点
                    if (edge[0] == nodetemp) & (edge[1] != nodetemp):
                        if edge[1] not in editednode:
                            stage[layertemp][0] = stage[layertemp][0] + 1
                            stage[layertemp][1].append(edge[1])
                            nodestage[edge[1]] = [nodetemp,layertemp]
                            nodes.remove(edge[1])
                            editednode.append(edge[1])
            if len(editednode) > editnodetemp:
                layertemp = layertemp + 1
                stage[layertemp] = [0,[]]
            else:
                break
        return [stage,nodestage]
...
Asmverify.jp_analysis()
Asmverify.jpdgraph = Asmverify.dgraph_repair(Asmverify.jpdgraph)
stage = Asmverify.degraphpos_repair(Asmverify.jpdgraph,Asmverify.jpdgraphpos,'DD8IU')
